...
public class HtmlReportGenerator {
    ...
    public static File generateHtmlReport(List<ComparisonResult> results, String outputPath) throws IOException {
        ...
        // If the provided path is a directory, append a default filename to determine the base name.
        ...
        try (PrintWriter writer = new PrintWriter(new FileWriter(htmlFile))) {
            ...
            writer.println("<title>API Comparison Report</title>");
            writer.println(getStyles());
            writer.println("</head>");
            writer.println("<body>");
            writer.println("<h1>API Comparison Report</h1>");

            // --- Summaries ---
            String generationTimestamp = ZonedDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss z"));
            ...
            writer.println("<thead><tr><th>#</th><th>Operation</th><th>Status</th><th>Tokens</th><th>Details</th></tr></thead>");
            ...
            for (ComparisonResult result : results) {
                ...
                writer.println("<td><button onclick=\"toggleDetails('details-" + iteration + "')\">Toggle Details</button></td>");
                writer.println("</tr>");
                ...
                if ("ERROR".equals(result.getStatus())) {
                    writer.println("<p class=\"error-message\"><strong>Error:</strong> " + escapeHtml(result.getErrorMessage()) + "</p>");
                }
    ...
    private static void generateSummary(PrintWriter writer, List<ComparisonResult> results, String generationTimestamp) {
        ...
        long totalDuration = results.stream()
                .mapToLong(r -> (r.getApi1() != null ? r.getApi1().getDuration() : 0) + (r.getApi2() != null ? r.getApi2().getDuration() : 0))
                .sum();
